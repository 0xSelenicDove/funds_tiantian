...
2. **自选列表四栏表格布局 (Watchlist Table)**
   * 启动默认进入自选列表，采用四栏表格化布局（基金名称、天天估值、自己估值、最新涨跌）展示。
   * **天天估值**：拉取天天官方日内最新估值变动百分比（`gszzl`）。
   * **自己估值**：基于前十大重仓持仓及日内个股实时走势（支持自选估值模型）实时预测计算。
   * **最新涨跌**：展示该基金最新已公布的实际日净值涨跌幅百分比，并在下方小字括号内标注**更新日期**（例如 `(05-29)`），实现今日双估值与昨日实际涨跌幅一目了然的侧边直观对比。
   * 支持一键星标收藏，并支持在“管理”模式下通过**鼠标拖拽**或**手机屏幕触控拖动 (Touch Drag & Drop)** 实时调整自选基金的排列顺序或删除基金。
...
7. ** premium 动效设计与可视化图表 (ECharts & M3)**
   ...
   * 高端 Android Material Design 3 风格明亮白调设计，支持流畅的淡入淡出、微交互动效和原生般的手势/拖拽体验。
...
为了使得估值尽可能贴近晚间官方公布的真实净值，本应用在“重仓+指数”模型下，从天天基金资产配置历史页面自动爬取最新截止的整体资产构成，提取 `股票配置占比` (记作 $P_{stock}$)。
